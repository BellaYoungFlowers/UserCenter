...
import com.example.myusercenterback.utils.AlgorithmUtils;
...
//	}
    ...
    public List<User> matchUsers(int num, User loginUser) {
        ...
        List<User> allUserslist = this.list();
        SortedMap<Integer, Integer> minDistanceMap = new TreeMap<>();
        for (int i = 0; i < allUserslist.size(); i++) {
            String currentUserTags = allUserslist.get(i).getTags().toString();
            if(StringUtils.isBlank(currentUserTags)){
                continue;
            }
            List<String> currentUserTagsList = gson.fromJson(currentUserTags, new TypeToken<List<String>>() {}.getType());
			int distance = AlgorithmUtils.minDistance(userTagsList, currentUserTagsList);
			minDistanceMap.put(i, distance);
        }

        List<Integer> indexList = minDistanceMap.keySet().stream().limit(5).collect(Collectors.toList());
        List<User> userList = indexList.stream().map(index -> getSafetyUser(allUserslist.get(index))).collect(Collectors.toList());
        return userList;
    }
}
